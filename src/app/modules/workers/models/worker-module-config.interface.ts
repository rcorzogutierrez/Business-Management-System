...
export interface ModulePermissions {
  canView: boolean;                // Ver trabajadores
  canCreate: boolean;              // Crear trabajadores
  canEdit: boolean;                // Editar trabajadores
  canDelete: boolean;              // Eliminar trabajadores
  ...
  canAssign: boolean;              // Asignar trabajadores a proyectos
}
...
/**
 * Configuración completa del módulo de trabajadores
 */
export interface WorkerModuleConfig {
  // Identificación
  id: string;                      // ID de la configuración (único)
  moduleId: 'workers';             // ID del módulo
  ...
  settings: {
    ...
    enableAssignment: boolean;     // Habilitar asignación de trabajadores
    enableStatus: boolean;         // Habilitar estados personalizados
    requireApproval: boolean;      // Requerir aprobación para crear/editar
    autoArchive: boolean;          // Archivar automáticamente trabajadores inactivos
 ...
 */
export const DEFAULT_MODULE_CONFIG: Omit<WorkerModuleConfig, 'id' | 'fields' | 'lastModified' | 'modifiedBy'> = {
  moduleId: 'workers',
